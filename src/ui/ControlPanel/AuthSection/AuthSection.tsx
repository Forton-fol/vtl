...
export function AuthSection(props: AuthSectionProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (patreonConnected === 'connected') {
      toast.notify({
        title: t('register.patreonConnected') || 'Patreon подключен',
        message: 'Подключение Patreon прошло успешно.',
        type: 'success',
      });
  ...
  async function doRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res && res.user) {
      toast.notify({
        title: t('register.success') || 'Registered successfully',
      ...
      toast.notify({
        title: t('register.captchaError') || 'Captcha required',
  ...
  async function doPatreonConnect() {
    ...
    if (res && 'error' in res) {
      toast.notify({
        ...
        message: 'Please log in before connecting Patreon.',
        type: 'warning',
      });
    }
  }
